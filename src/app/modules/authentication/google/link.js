...
exports = module.exports = function(User, mailer) {
	return function(req, token, refreshToken, profile, done) {
		User.findOne({
			...
			if (user) {
				return done(null, req.user, req.flash('errorMessage', 'This Google account is already linked to an account in our system. Please unlink it from the existing to be able to re-link.'));
			}
